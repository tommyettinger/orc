(ns orc.connection
  ...
  (:use [orc.routing :only [dispatch-command]]
  	[overtone.at-at]))

;; todo: automatically generate names when there's a name clash
(def NICK "RL_Bot")
(def USER-NAME "RL Bot")
(def HOST-NAME "RL_Bot")
(def SERVER-NAME "RL_Bot")
(def REAL-NAME "A SimpleRL Bot that only works for people using certain clients (like Floe)")
